...
const CATEGORIES: Category[] = [
  ...
  },
  {
    value: "adventures",
    label: "Adventures",
    desc: "Active & off-the-beaten-track",
    icon: (
      <svg viewBox="0 0 24 24" className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2}>
  ...
  },
  {
    value: "photography",
    label: "Photography",
    desc: "Golden hour & migration shots",
    icon: (
      <svg viewBox="0 0 24 24" className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2}>
...
// Accent colours per category (active state)
      ...
      <section className="bg-gradient-to-br from-teal-50 via-white to-stone-100 pt-20 pb-16 px-4 md:px-6 relative overflow-hidden border-b border-stone-100">
        ...
        <div className="relative max-w-7xl mx-auto">
          <div className="max-w-2xl">
            <p className="eyebrow text-teal-DEFAULT mb-4">No hidden fees. Ever.</p>
            ...
            <p className="text-gray-500 text-lg leading-relaxed">
              Every price shown is the real price. Book online in minutes — no forms, no waiting, no negotiation.
            </p>
          </div>
        </div>
      </section>
          ...
          {activeCat && activeCat.value && (
            <div className="mt-5 flex items-center gap-3">
              ...
              <p className="text-gray-500 text-sm">
                Showing <span className="font-semibold text-gray-800">{activeCat.label}</span> safaris
                {safaris.length > 0 && <> · <span className="text-teal-DEFAULT font-semibold">{safaris.length} package{safaris.length !== 1 ? "s" : ""}</span></>}
              </p>
            </div>
          )}
        </div>
