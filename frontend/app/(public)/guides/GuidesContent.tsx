...
export default function GuidesContent({ guides }: Props) {
  const { t } = useLanguage();

  const TRUST_POINTS = [
    { icon: TRUST_ICONS[0], labelKey: "guides.traLicensed" as const },
    { icon: TRUST_ICONS[1], labelKey: "guides.backgroundChecked" as const },
    { icon: TRUST_ICONS[2], labelKey: "guides.minExperience" as const },
    { icon: TRUST_ICONS[3], labelKey: "guides.avgRating" as const },
  ];
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-teal-50 via-white to-stone-100 pt-20 pb-20 px-4 md:px-6 border-b border-stone-100">
        <div className="max-w-7xl mx-auto">
          <div className="max-w-3xl">
            <p className="eyebrow text-teal-DEFAULT mb-4">{t("guides.subtitle")}</p>
            ...
            <div className="flex flex-wrap gap-3">
              {TRUST_POINTS.map(({ icon, labelKey }) => (
                <div key={labelKey} className="flex items-center gap-2 bg-white border border-stone-200 shadow-sm px-4 py-2 rounded-full text-sm text-gray-700">
          ...
          {guides.length > 0 && (
            <div className="grid grid-cols-3 gap-px bg-stone-200 rounded-2xl overflow-hidden mt-14 max-w-lg">
              {[
                { value: `${guides.length}`, label: t("guides.expertGuides") },
                { value: `${totalYears}+`, label: t("guides.combinedYears") },
                { value: `${totalReviews}+`, label: t("guides.guestReviews") },
              ].map(({ value, label }) => (
      ...
      {featured.length > 0 && (
        <section className="py-16 px-4 md:px-6 max-w-7xl mx-auto">
          <p className="eyebrow text-teal-DEFAULT mb-8">{t("guides.mostRequested")}</p>
      ...
      {rest.length > 0 && (
        <section className="pb-20 px-4 md:px-6 max-w-7xl mx-auto">
          <p className="eyebrow text-teal-DEFAULT mb-8">{t("guides.allGuides")}</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {rest.map((guide) => (
              <GuideCard key={guide.id} guide={guide} />
            ))}
      ...
      {guides.length === 0 && (
        <section className="py-32 text-center text-gray-400">
          <p className="text-lg">{t("guides.comingSoon")}</p>
        </section>
      )}
      ...
      <section className="bg-stone-50 border-t border-stone-200 py-16 px-4 md:px-6 text-center">
        <div className="max-w-xl mx-auto">
          <h2 className="font-display text-3xl font-bold text-gray-900 mb-3">
            {t("guides.requestGuide")}
          </h2>
          <p className="text-gray-500 text-sm mb-6 leading-relaxed">
            {t("guides.haveGuide")} Mention them in your enquiry and we will do our best to assign them to your safari.
          </p>
          <Link
            href="/enquire"
